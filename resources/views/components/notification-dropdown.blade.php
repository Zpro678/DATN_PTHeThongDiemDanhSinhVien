...
<div class="relative" 
            ...
            if (window.Echo && userId) {
                window.Echo.private('App.Models.User.' + userId)
                    .listen('.NotificationEvent', (e) => {
                        ...
                        let newNotif = {
                            href: e.notification.url || '#',
                            title: e.notification.title,
                            message: e.notification.message,
                            time: 'Vừa xong',
                            unread: true,
                            level: e.notification.type === 'leave_request_rejected' ? 'danger' 
                                 : (e.notification.type === 'leave_request_approved' ? 'success' : 'info')
                        };
    ...
    @click.away="openNotification = false; showAll = false">
    <button
        type="button"
    ...
    >
        ...
        @if ($showIndicator)
            <span class="{{ $indicatorClass }}" x-show="hasUnread"></span>
        @endif
    </button>
        ...
        <div class="scrollbar-custom max-h-[500px] overflow-y-auto overscroll-contain">
            <template x-for="(notification, index) in notifications" :key="index">
                <a
                    ...
                    :class="{
                        'flex items-start gap-4 px-4 py-3 transition-colors hover:bg-slate-50': true,
                        'border-b border-slate-50': index !== notifications.length - 1,
                        'opacity-70': notification.read === true
                    }"
                >
                    <div class="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full"
                         :class="notification.level === 'danger' ? 'bg-rose-100 text-rose-600' : (notification.level === 'success' ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-100 text-blue-600')">
                        <template x-if="notification.level === 'danger'"><x-user.icon name="alert-triangle" :size="16" /></template>
                        <template x-if="notification.level === 'success'"><x-user.icon name="check-circle" :size="16" /></template>
                        <template x-if="!['danger', 'success'].includes(notification.level)"><x-user.icon name="info" :size="16" /></template>
                    </div>

                    <div class="flex-1 space-y-1">
                        <p class="text-sm font-medium text-slate-900" x-text="notification.title || 'Thông báo'"></p>
                        <p class="text-xs text-slate-500" x-text="notification.message || ''"></p>
                        <p class="text-[10px] font-medium text-slate-400" x-text="notification.time || ''"></p>
                    </div>

                    <template x-if="notification.unread">
