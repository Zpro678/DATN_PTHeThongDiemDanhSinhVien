@props([
    ...
    'dispatch' => null, // Tên event Alpine/Livewire để dispatch (dành cho nút nằm ngoài Livewire component).
    'label' => 'Xuất Excel',
    ...
    'navigate' => true, // Tắt với link tải file để Livewire không chặn download.
])
...
@php
    ...
    $wireTarget = $target ?? $action ?? $dispatch;
@endphp
...
@elseif($href)
    <a href="{{ $href }}" @if($navigate) wire:navigate @endif {{ $attributes->merge(['class' => $enabledClass]) }}>
        <x-user.icon name="download" :size="16" />
        <span class="leading-none">{{ $label }}</span>
    </a>
@elseif($dispatch)
    <button type="button" @click="$dispatch('{{ $dispatch }}')" {{ $attributes->merge(['class' => $enabledClass]) }}>
