    ...
    <div class="flex min-h-[calc(100vh-8rem)] flex-col space-y-5">
        <div class="admin-card overflow-hidden rounded-3xl border p-5 lg:p-6 flex-none">
            ...
            <div class="flex gap-3">
                ...
                <button type="button" class="admin-soft-button inline-flex items-center rounded-xl border border-transparent bg-gradient-to-r from-blue-600 to-cyan-600 px-4 py-2.5 text-sm font-bold text-white shadow-sm shadow-blue-500/20">
        ...
        <div class="admin-card flex-none overflow-hidden rounded-2xl border p-4 lg:p-5">
            <form action="{{ route('admin.users.index') }}" method="GET" class="grid grid-cols-1 gap-4 md:grid-cols-4">
                ...
                <div>
                    <select name="role" onchange="this.form.submit()" class="relative z-10 block w-full rounded-xl border-slate-200 bg-white py-2.5 pl-3 pr-10 text-sm text-slate-900 transition-colors focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    <select name="status" onchange="this.form.submit()" class="relative z-10 block w-full rounded-xl border-slate-200 bg-white py-2.5 pl-3 pr-10 text-sm text-slate-900 transition-colors focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div class="admin-card flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border">
                            ...
                            <tr class="transition-colors hover:bg-blue-50/40 {{ $user->status === 'blocked' ? 'opacity-75' : '' }}">
                                <td class="whitespace-nowrap px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-100 text-sm font-black text-blue-700 {{ $user->status === 'blocked' ? 'grayscale' : '' }}">
                                            {{ $initial }}
                                        </div>
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                                    ...
                                    <button type="button" class="{{ $user->status === 'active' ? 'text-red-600 hover:text-red-900' : 'text-green-600 hover:text-green-900' }} transition-colors" title="{{ $user->status === 'active' ? 'Khóa tài khoản' : 'Mở khóa tài khoản' }}">
