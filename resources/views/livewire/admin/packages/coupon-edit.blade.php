        ...
        <div class="mb-6 p-5 lg:p-6">
            <div class="relative z-10 flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
                <div>
                    ...
                    <h1 class="mb-1 text-[28px] font-bold text-slate-900">Cập nhật mã giảm giá</h1>
                    <p class="text-sm text-slate-500">Chỉnh sửa thông số cho mã giảm giá {{ $coupon->code }}.</p>
                </div>
            ...
            <form wire:submit="save" class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="number" wire:model="value" min="0" @if($type === 'PERCENT') max="100" step="0.01" @endif placeholder="{{ $type === 'PERCENT' ? 'VD: 20' : 'VD: 50000' }}" class="w-full rounded-xl border border-slate-300 bg-transparent px-4 py-2.5 text-slate-900 transition-colors focus:border-blue-500 focus:ring-2 focus:ring-blue-500">
                        @if($type === 'PERCENT')
                            <span class="mt-1 block text-xs text-slate-400">Giảm theo % — tối đa 100%.</span>
                        @endif
