<div class="w-full px-6 py-6 pb-24 sm:px-10 lg:px-16 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    @php
    $validSessions = $attendanceDetail['attended'] ?? (($attendanceDetail['present'] ?? 0) + ($attendanceDetail['late']
    ?? 0) + ($attendanceDetail['excused'] ?? 0));
    ...
    <div class="mt-6 mb-8 relative overflow-hidden rounded-xl {{ $themeBgClass }} shadow-sm">
        ...
        {{-- Background Icon --}}
        <div
            class="absolute right-0 top-1/2 z-0 opacity-15 pointer-events-none transform -translate-y-1/2 translate-x-1/4 sm:translate-x-0 sm:right-10">
        ...
        <div class="flex flex-col h-full min-h-[160px] sm:min-h-[180px] px-6 py-6 sm:px-8 lg:px-10 relative z-10">
            <div class="flex justify-between items-start gap-4">
                <div class="pr-2">
                    <h1
                        class="text-3xl sm:text-[2.5rem] leading-tight font-medium tracking-tight text-white drop-shadow-sm">
                        {{ $class->name }}</h1>
    ...
    <div class="flex flex-col gap-6 lg:flex-row items-stretch">
        ...
        <div class="flex flex-col lg:w-1/3 xl:w-1/4">
            <div
                class="flex flex-col h-full rounded-2xl bg-white p-5 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] border border-slate-100">
                <div class="mb-5 flex items-center gap-3">
                    <div
                        class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 {{ $themeTextClass }}">
                ...
                <div class="flex flex-col gap-2.5 text-[13px] sm:text-sm">
                    ...
                    <!-- Nhóm Giảng viên & Trợ giảng -->
                    <div class="flex flex-col gap-2.5 rounded-xl bg-[#f8faff] p-4 transition hover:bg-[#f0f4ff]">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <span class="font-bold text-slate-800 text-right leading-tight max-w-[55%]">
                                {{ $class->owner->name ?? 'Chưa cập nhật' }}
                            </span>
                        </div>

                        <!-- Danh sách Đồng chủ lớp (chỉ hiển thị nếu có) -->
                        @if($class->coOwners && $class->coOwners->count() > 0)
                        <div class="mt-1 border-t border-slate-200/60 pt-3 flex flex-col gap-2.5">
                            @foreach($class->coOwners as $coOwner)
                            <div class="flex items-center justify-between pl-1">
                                <div class="flex items-center gap-2">
                                    <x-user.icon name="users" :size="14" class="text-slate-400" />
                                    <span class="text-xs font-medium text-slate-500">Trợ giảng</span>
                                </div>
                    ...
                    <!-- Mã lớp -->
                    <div
                        class="flex items-center justify-between rounded-xl bg-[#f8faff] px-4 py-3 transition hover:bg-[#f0f4ff]">
                        <div class="flex items-center gap-3">
                            <x-user.icon name="qr-code" :size="16" class="text-slate-600" />
                            <span class="font-medium text-slate-500">Mã lớp</span>
                        </div>
                        <span
                            class="font-bold text-slate-800 text-right">{{ $class->class_code ?? $class->join_key }}</span>
                    </div>
                    <div class="flex items-center justify-between py-3 border-b border-slate-100">
                        <span class="text-sm text-slate-500">Mã tham gia</span>
                        <span
                            class="font-bold text-slate-800 text-right font-mono tracking-widest text-blue-600">{{ $class->join_key }}</span>
                    </div>
        ...
        <div class="flex flex-1 flex-col gap-6">
            ...
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-3">
                ...
                $tiles = [
                ['label' => 'CHUYÊN CẦN', 'value' => $percent.'%', 'icon' => 'trending-up', 'color' => 'blue', 'border'
                => 'border-blue-100', 'bg' => 'bg-[#f4f7fe]', 'extra' => 'shadow-sm'],
                ['label' => 'BUỔI HỢP LỆ', 'value' => $validSessions, 'icon' => 'check-circle', 'color' => 'emerald',
                'border' => 'border-emerald-100', 'bg' => 'bg-[#f0fdf4]', 'extra' => 'shadow-sm'],
                ['label' => 'TỔNG BUỔI', 'value' => $attendanceDetail['total'] ?? 0, 'icon' => 'calendar-check', 'color'
                => 'slate', 'border' => 'border-slate-100', 'bg' => 'bg-[#f8fafc]', 'extra' => 'shadow-sm'],
                ['label' => 'BUỔI ĐI MUỘN', 'value' => $attendanceDetail['late'] ?? 0, 'icon' => 'clock', 'color' =>
                'amber', 'border' => 'border-amber-100', 'bg' => 'bg-[#fffbeb]', 'extra' => 'shadow-sm'],
                ['label' => 'BUỔI VẮNG', 'value' => $attendanceDetail['absent'] ?? 0, 'icon' => 'x-circle', 'color' =>
                'rose', 'border' => 'border-rose-100', 'bg' => 'bg-[#fff1f2]', 'extra' => 'shadow-sm'],
                ['label' => 'QUỸ VẮNG', 'value' => $absenceBudgetValue, 'icon' => $absenceBudgetState === 'safe' ?
                'shield-check' : 'alert-triangle', 'color' => $budgetColor, 'border' => $budgetBorder, 'bg' =>
                $budgetBg, 'extra' => $budgetExtra, 'status_text' => $budgetTag],
                ];
                @endphp
            ...
            <div class="rounded-2xl border border-slate-100 bg-white p-5 sm:p-6 shadow-sm">
                <div class="flex items-start justify-between mb-6">
                    <div>
                        ...
                        <p class="mt-0.5 text-[13px] text-slate-500">Duy trì trên mức tối thiểu để đủ điều kiện dự thi.
                        </p>
                    </div>
                    <div class="text-right hidden sm:block">
                        <div class="text-[2rem] leading-none font-black {{ $themeTextClass }}">{{ $percent }}%</div>
                        <div class="text-[10px] font-bold uppercase tracking-widest text-slate-500 mt-1.5">CURRENT
                            STATUS</div>
                    </div>
                </div>
                ...
                <div
                    class="mt-6 flex items-start gap-3.5 rounded-xl bg-slate-50 px-5 py-4 text-[13px] text-slate-600 border border-slate-100">
