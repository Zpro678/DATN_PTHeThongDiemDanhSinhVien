<div class="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    {{-- Breadcrumb & Header --}}
    <div class="mb-6">
        <div class="mb-3 flex items-center gap-2 text-sm text-on-surface-variant">
            <a href="{{ route('managed-classes') }}" wire:navigate class="hover:text-primary transition-colors">Lớp tôi quản lý</a>
            <x-user.icon name="chevron-right" :size="14" />
            <a href="{{ route('lecturer.classes.show', $courseClass) }}" wire:navigate class="hover:text-primary transition-colors">{{ $courseClass->name }}</a>
            <x-user.icon name="chevron-right" :size="14" />
            <span class="text-on-surface font-medium">Cài đặt</span>
        </div>
    ...
    <form wire:submit="save" id="class-settings-form">
        <div class="flex flex-col lg:flex-row gap-6 items-start">
            ...
            <div class="flex-1 min-w-0 space-y-6">
                ...
                <div class="rounded-2xl border border-outline-variant/20 bg-white shadow-sm overflow-hidden">
                    <div class="flex items-center gap-3 border-b border-outline-variant/10 bg-surface-container-lowest/50 px-6 py-4">
                        ...
                        <h2 class="text-base font-bold text-on-surface">Thông tin chung</h2>
                    </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center gap-3">
                            ...
                            <button type="button" wire:click="regenerateCode"
                                class="shrink-0 inline-flex items-center gap-2 rounded-xl border border-outline-variant/30 bg-surface-container-lowest px-4 py-3 text-sm font-bold text-on-surface-variant hover:bg-primary hover:text-white hover:border-primary transition-colors"
                ...
                <div class="rounded-2xl border border-outline-variant/20 bg-white shadow-sm">
                    ...
                    <div class="p-6 space-y-5">
                        ...
                        <div class="flex items-center justify-between rounded-xl border border-outline-variant/20 bg-surface-container-lowest/60 p-4">
                            <div class="pr-4">
            ...
            <div class="w-full lg:w-72 xl:w-80 shrink-0 space-y-4 lg:sticky lg:top-6">
                ...
                <div class="rounded-2xl border border-outline-variant/20 bg-white shadow-sm">
                    <div class="flex items-center gap-3 rounded-t-2xl border-b border-outline-variant/10 bg-surface-container-lowest/50 px-5 py-3.5">
                        ...
                        <h2 class="text-sm font-bold text-on-surface">Trạng thái lớp</h2>
                    </div>
                    <div class="p-4"
                        x-data="{
                            open: false,
                        ...
                        <div class="relative">
                            <button type="button" @click="open = !open; if(open) checkPosition();" x-ref="btn"
                                class="flex w-full items-center justify-between gap-3 rounded-xl border border-outline-variant/30 bg-surface-container-lowest px-4 py-3 text-left text-sm font-bold text-on-surface outline-none transition-all hover:border-primary"
                                :class="open ? 'border-primary ring-2 ring-primary/20' : ''">
                            ...
                            <div x-show="open" x-cloak x-ref="menu"
                                ...
                                <template x-for="option in options" :key="option.value">
                                    <button type="button"
                                        @click="value = option.value; open = false"
                                        class="flex w-full items-center justify-between gap-3 px-4 py-3 text-sm font-bold transition-colors hover:bg-slate-50"
                                        :class="value === option.value ? 'text-primary bg-primary/5' : 'text-slate-700'">
                                        <span x-text="option.label"></span>
                                        <span x-show="value === option.value" class="flex h-4 w-4 items-center justify-center rounded-full bg-primary">
                                            <x-user.icon name="check" :size="10" class="text-white" />
                ...
                <div class="rounded-2xl border border-outline-variant/20 bg-white shadow-sm overflow-hidden">
                    ...
                    <div class="p-4">
                        <div class="flex items-start gap-3">
                            <div class="flex-1">
                                <span class="block text-sm font-bold text-on-surface">Yêu cầu duyệt thành viên</span>
                                <span class="text-xs text-on-surface-variant mt-0.5 block">Học viên phải chờ phê duyệt trước khi vào lớp.</span>
                            </div>
