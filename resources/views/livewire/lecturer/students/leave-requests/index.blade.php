...
<div class="w-full space-y-6 px-6 py-6 pb-24 sm:px-10 lg:px-16"
     x-data
     x-init="
        window.listenRealtime && window.listenRealtime('{{ (string) config('database.redis.options.prefix') }}private-App.Models.User.{{ auth()->id() }}', () => { $wire.$refresh() });
        (@js($this->realtimeChannels())).forEach(ch => window.listenRealtime && window.listenRealtime(ch, () => $wire.$refresh(), 300));
     ">
                    ...
                    @forelse ($leaveRequests as $request)
                        <tr class="transition-colors hover:bg-slate-50/70 cursor-pointer" onclick="if(!event.target.closest('a, button')) window.location.href='{{ route('lecturer.leave-requests.show', $request) }}'">
                            <td class="px-6 py-4"><div class="flex items-center gap-3">
                                ...
                                @endif
                                <span><a href="{{ route('lecturer.students.show', $request->classMember) }}" class="block text-sm font-bold text-slate-900 hover:text-primary">{{ $request->classMember->full_name }}</a><span class="text-xs text-slate-500">{{ $request->classMember->user?->email ?? $request->classMember->email }}</span></span></div></td>
