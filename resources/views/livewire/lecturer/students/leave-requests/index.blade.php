...
<div class="mx-auto max-w-[1400px] space-y-6 p-4 pb-24 sm:p-8">
    <section class="flex flex-col justify-between gap-4 rounded-[2rem] border border-outline-variant/10 bg-white p-6 shadow-sm md:flex-row md:items-end">
        <div>
            ...
            <p class="mt-2 text-sm text-slate-500">Theo dõi và xử lý đơn xin nghỉ của sinh viên trong các lớp bạn quản lý.</p>
        </div>
        <a href="{{ route('lecturer.students.index') }}" class="inline-flex items-center justify-center gap-2 rounded-full border border-slate-200 px-5 py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-50"><x-user.icon name="users" :size="18" />Quản lý sinh viên</a>
    </section>
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[1050px] text-left">
                <thead class="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500"><tr><th class="px-6 py-4">Sinh viên</th><th class="px-4 py-4">Lớp học</th><th class="px-4 py-4">Buổi xin nghỉ</th><th class="px-4 py-4">Lý do</th><th class="px-4 py-4">Ngày gửi</th><th class="px-6 py-4 text-right">Thao tác</th></tr></thead>
                <tbody class="divide-y divide-slate-100">
                    @forelse ($leaveRequests as $request)
                        <tr class="transition-colors hover:bg-slate-50/70">
                            ...
                            <td class="max-w-[280px] px-4 py-4"><p class="line-clamp-2 text-sm text-slate-600">{{ $request->reason }}</p>@if($request->proof_image)<a href="{{ asset('storage/'.$request->proof_image) }}" target="_blank" class="mt-1 inline-flex items-center gap-1 text-xs font-bold text-primary"><x-user.icon name="image" :size="14" />Minh chứng</a>@endif</td>
