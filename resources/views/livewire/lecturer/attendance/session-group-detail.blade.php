<div x-data="{ 
    ...
    get modalData() {
        ...
        const cell = this.matrix[this.selectedStudentId] || { status: 'pending', text: 'Chưa điểm danh', details: [] };
        return { student, sessionInfo: this.groupInfo, cell };
    },
    ...
    <section class="flex flex-col justify-between gap-6 md:flex-row md:items-end px-2">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('lecturer.attendance.index') }}" class="group inline-flex items-center justify-center gap-2 rounded-2xl bg-white px-5 py-2.5 text-sm font-bold text-slate-700 shadow-sm ring-1 ring-inset ring-slate-200 transition-all hover:bg-slate-50 hover:text-indigo-600 hover:ring-indigo-200">
                    ...
                    <div>
                        <div class="flex items-center gap-3 mb-2">
                            <span class="inline-flex items-center rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-600 uppercase tracking-widest" 
                                  x-text="groupInfo.startLesson && groupInfo.endLesson ? 'Tiết ' + groupInfo.startLesson + ' - ' + groupInfo.endLesson : groupInfo.lessonCount + ' tiết'"></span>
                            <span class="flex items-center gap-1.5 text-sm font-medium text-slate-500" x-show="groupInfo.timeStr">
                                <x-user.icon name="clock" :size="16" class="text-slate-400" />
                                <span x-text="groupInfo.timeStr"></span>
                            </span>
                        </div>
                        <h2 class="text-2xl font-black text-slate-900">
                            <span class="capitalize">{{ $sessions->first()->name }}</span> ngày <span class="text-indigo-600" x-text="groupInfo.date"></span>
                        </h2>
                    </div>
                ...
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead class="bg-transparent border-b border-slate-100">
                        <tr>
                            ...
                            <template x-if="sessionFilter === 'detailed'">
                                <template x-for="col in groupInfo.columns" :key="col.iteration">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <template x-for="(student, index) in drawerStudents" :key="student.id">
                            <tr class="group hover:bg-slate-50/60 transition-colors">
                                ...
                                <td class="py-5 align-middle transition-all duration-300" :class="sessionFilter === 'merged' ? 'pl-20 pr-8' : 'px-8'">
                                    <div class="flex items-center gap-4">
                                        <div class="flex h-10 w-10 items-center justify-center rounded-2xl text-sm font-bold transition-colors border group-hover:opacity-80" 
                                             :class="`${student.avatar_bg || 'bg-slate-100'} ${student.avatar_text || 'text-slate-600'} ${student.avatar_border || ''}`" 
                                ...
                                <template x-if="sessionFilter === 'detailed'">
                                    <template x-for="col in groupInfo.columns" :key="col.iteration">
                                        <td class="px-4 py-5 align-middle text-center cursor-pointer" @click="selectedStudentId = student.id; showModal = true; scrollToTop()">
                                ...
                                <td class="px-8 py-5 text-center align-middle">
                                    ...
                                    <span x-show="student.cell.status === 'absent'" class="inline-flex min-w-[110px] items-center justify-center rounded-xl px-4 py-2 text-[13px] font-bold bg-rose-50 text-rose-600 border border-rose-100 shadow-sm">Vắng mặt</span>
                                    <span x-show="student.cell.status === 'late'" class="inline-flex min-w-[110px] items-center justify-center rounded-xl px-4 py-2 text-[13px] font-bold bg-amber-50 text-amber-600 border border-amber-100 shadow-sm">Đi trễ</span>
                            ...
                            <div class="px-6 py-6 space-y-4">
                                ...
                                <div class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50/50 border border-slate-100">
                                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl text-lg font-black border"
                                         :class="`${modalData.student.avatar_bg || 'bg-slate-100'} ${modalData.student.avatar_text || 'text-slate-600'} ${modalData.student.avatar_border || ''}`">
