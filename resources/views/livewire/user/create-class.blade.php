    ...
    <div class="w-full max-w-none space-y-6">
        <div class="flex flex-col gap-4 rounded-[28px] border border-white/80 bg-white/80 p-5 shadow-sm shadow-slate-200/70 backdrop-blur sm:p-6 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex items-start gap-4">
                ...
                <div>
                    <p class="text-xs font-bold uppercase tracking-[0.28em] text-blue-600">Quản lý lớp học</p>
                    <h1 class="mt-2 text-2xl font-extrabold uppercase leading-tight tracking-tight text-slate-950 sm:text-3xl">Thêm lớp học mới</h1>
                    <p class="mt-2 max-w-3xl text-sm text-slate-500">Khởi tạo thông tin lớp học và các cấu hình điểm danh. Bố cục được mở rộng để dùng trọn không gian làm việc của hệ thống.</p>
                </div>
            </div>

            <a href="{{ route('managed-classes') }}" class="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 shadow-sm transition-colors hover:bg-slate-50">
                ...
                <div class="sticky top-6 space-y-6">
                            ...
                            <div class="rounded-2xl border border-slate-200 p-4">
                                <div class="flex items-center gap-3">
                                    <div @class([
                                        ...
                                        'bg-blue-50 text-blue-600' => $requireApproval,
                                        'bg-slate-100 text-slate-500' => ! $requireApproval,
                                    ])>
                                        <x-user.icon name="shield-check" :size="20" />
                                    </div>
                                    <div>
                    ...
                    <section class="rounded-[28px] border border-slate-200/80 bg-white p-6 shadow-sm">
                        <h3 class="flex items-center gap-2 text-base font-extrabold text-slate-950">
                            <x-user.icon name="sparkles" :size="19" class="text-amber-500" />
                            Gợi ý nhập liệu
                        </h3>
                        <div class="mt-5 space-y-4">
                            <div class="flex gap-3">
                                <span class="mt-0.5 flex h-6 w-6 items-center justify-center rounded-full bg-blue-50 text-xs font-extrabold text-blue-600">1</span>
                                <p class="text-sm leading-6 text-slate-600">Mã lớp nên ngắn, dễ nhận diện và không trùng với lớp đã có.</p>
                            </div>
                            <div class="flex gap-3">
