<div class="flex min-h-screen">
    <aside class="fixed left-0 top-0 z-40 hidden h-screen w-64 shrink-0 flex-col overflow-hidden border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 xl:flex">
        ...
        <nav class="sams-scrollbar min-h-0 flex-1 space-y-7 overflow-y-auto px-4 py-6">
    ...
    <div x-cloak x-show="sidebarOpen" class="fixed inset-0 z-50 flex xl:hidden" x-transition.opacity>
        ...
        <aside class="relative flex h-screen w-64 flex-col overflow-hidden border-r border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full">
            ...
            <nav class="sams-scrollbar min-h-0 flex-1 space-y-6 overflow-y-auto">
                @foreach ($adminMenuGroups as $group)
                    <div class="space-y-1">
                        ...
                        @foreach ($group['items'] as $item)
                            ...
                            <a href="{{ url($item['href']) }}" class="{{ $active ? 'bg-blue-600 text-white shadow-sm shadow-blue-500/10' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800' }} group flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-xs font-bold transition-all" @click="sidebarOpen = false">
                                <span class="flex items-center gap-2.5">
                                ...
                                @isset($item['badge'])
                                    <span class="{{ $active ? 'bg-blue-700/80 text-white' : 'bg-slate-100 text-slate-500 dark:bg-slate-700' }} rounded-md px-1.5 py-0.5 text-[10px] font-extrabold">
                                        {{ $item['badge'] }}
                                    </span>
                                @endisset
                            </a>
